...
namespace Mario_s_Activator.Misc
{
    internal class Drawings
    {
        ...
        {
            if (SummonerSpells.PlayerHasSmite && SummonerSpells.Smite.IsReady() && MyMenu.SummonerMenu.GetCheckBoxValue("drawSmiteDamage"))
            {
                foreach (
                    var jg in
                        EntityManager.MinionsAndMonsters.GetJungleMonsters()
                            .Where(m => m.IsValidTarget(SummonerSpells.Smite.Range + 400) && m.IsHPBarRendered && !m.Name.Contains("Mini")))
                {
        ...
        {
            /*
            if (MyMenu.DrawingMenu.GetCheckBoxValue("disableDrawings")) return;

            if (SummonerSpells.PlayerHasSmite)
            {
                if (SummonerSpells.Smite.IsReady() && !MyMenu.SummonerMenu.GetKeyBindValue("smiteKeybind") && MyMenu.SummonerMenu.GetCheckBoxValue("drawSmiteRange"))
                {
            ...
            foreach (
                var item in
                    Offensive.OffensiveItems.Where(i => i.IsReady() && i.Range > 0)
                        .Where(item => MyMenu.DrawingMenu.GetCheckBoxValue("draw" + (int) item.Id)))
            {
            ...
            if (MyMenu.SettingsMenu.GetCheckBoxValue("dev"))
            {
